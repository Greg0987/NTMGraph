...
class GSM:
    ...
    def train(self, train_data, batch_size=256, learning_rate=1e-3, test_data=None, num_epochs=100, is_evaluate=False,
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            for iter, data in enumerate(data_loader):
                ...
                bows_recon, mus, log_vars=self.vae(bows,
                                                   lambda x: torch.softmax(x, dim=1))  # 输入BOW到VAE里面，得到重建loss，分布的mu和var
                # 算VAE的重建loss
            ...
            if (epoch + 1) % log_every == 0:
                ...
                checkpoint={"net": self.vae.state_dict(), "optimizer": optimizer.state_dict(), "epoch": epoch,
                    "param": {"bow_dim": self.bow_dim, "n_topic": self.n_topic, "taskname": self.taskname}}
                ...
                plt.savefig(f'results/{self.taskname}/gsm_trainloss.png')
                if test_data != None:
        ...
        plt.legend()
        plt.savefig(
            f'results/{self.taskname}/gsm_tc_scores.png')  # The code lines between this and the last comment lines are duplicated with WLDA.py, consider to simpify them.
