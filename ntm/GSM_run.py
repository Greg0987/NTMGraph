...
def main():
    ...
    # checkpoint={"net": model.vae.state_dict(), "optimizer": model.optimizer.state_dict(), "epoch": num_epochs,
    #            "param": {"bow_dim": model.bow_dim, "n_topic": n_topic, "taskname": taskname}}
    # torch.save(checkpoint, save_name)



    torch.save(model.vae.state_dict(), save_name)
    print(f"Succesfully saved model. Model name: {save_name}.")
    ...
    top_words = model.show_topic_words(dictionary=docSet.dictionary)

    with open(f'results/{taskname}/results.txt', 'w', encoding='utf-8') as fp:
        for i, topic in enumerate(top_words):
            fp.write(f'topic{i}: {topic}\n')
    print(f'Finished writing results to results/{taskname}_results.txt')
